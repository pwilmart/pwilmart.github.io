...
There may be situations where some way to allocate channels to tags that minimizes cross talk is important. An 18-plex kit can be split into a 10-plex series and an 8-plex series that each have reduced cross talk. **Figure 3** shows the two series (10-plex on top and 8-plex on bottom). The cross talk from the satellite peaks associated with carbon atoms can be eliminated. However, the small cross talk of N-series tags to lighter C-series tags cannot be eliminated. However, the degree of that cross talk is only about a 1% relative effect.
